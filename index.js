import { activarBotonesAgregar } from "./cart.js";
      ...
      card.innerHTML = `
        ...
        <div class="mt-4 flex items-center justify-between">
          ...
          <div class="flex gap-2 items-center">
            <button id="add-to-cart" class="bg-orange-400 text-white px-3 py-1 border rounded-md text-sm">Agregar</button>
  ...
  if (saludos.some(saludo => query.includes(saludo))) {
    ...
    document.querySelectorAll('.card').forEach(card => {
      ...
      if (name.includes(query)) {
        ...
        found = `<strong>${card.dataset.name}</strong> cuesta ${price} descripcion: ${desc}`;
      }
    });
